...
Ochrana ATP v programu Azure se skládá z cloudové službě ochrana ATP v programu Azure, který se skládá z portálu pro správu pracovního prostoru a portálu pracovního prostoru, samostatný senzor ochrany ATP v programu Azure a/nebo senzoru služby Azure ATP. Další informace o součástech ochrany ATP v programu Azure najdete v tématu [architektura služby Azure ATP](atp-architecture.md).

Každý pracovní prostor ochrany ATP v programu Azure podporuje hranici doménové struktury služby Active Directory a doménovou strukturu funkční úroveň (ffl) v systémech Windows 2003 a novějších. U nasazení s více doménovými strukturami se vyžaduje pro každou doménovou strukturu samostatný pracovní prostor služby Azure ATP.
...
[Azure portálu pro správu pracovního prostoru ochrana ATP v programu](#azure-atp-workspace-management-portal-and-workspace-portal-requirements): Tato část popisuje požadavky na pracovní prostor Správa portálu prohlížeč.
...
-   Pokud spustíte Wireshark na služby Azure ATP samostatný senzor, je potřeba restartovat senzor rozšířené ochrany před internetovými útoky pro Azure Service po zastavení zaznamenávání Wireshark. V opačném případě senzor Zastaví zachytávání provozu.
...
-   Brány firewall a proxy open - komunikovat s cloudovou službou ochrany ATP v programu Azure, musíte mít otevřít: *. atp.azure.com port 443 v brány firewall a proxy. 
...
Samostatný senzor komunikovat s cloudovou službou, musíte otevřít port 443 v bran firewall a proxy servery na *. atp.azure.com
